    ...
    if (safe_str_neq(data_set->placement_strategy, "default")) {
        ...
        for (gIter = data_set->nodes; gIter != NULL; gIter = gIter->next) {
            ...
            if (have_enough_capacity(node, rsc) == FALSE) {
                ...
                /* キャパを持たないノードでは対象リソースの配置不可のスコアを設定する */
...
native_color(resource_t * rsc, node_t * prefer, pe_working_set_t * data_set)
{
    ...
    for (gIter = rsc->rsc_cons; gIter != NULL; gIter = gIter->next) {
        ...
        /* colocation情報のwith-rsc指定のリソース情報を取得する */
...
/* with-rsc指定されているリソースのスコアをrsc指定のリソースへ反映する */
void
native_rsc_colocation_lh(resource_t * rsc_lh, resource_t * rsc_rh, rsc_colocation_t * constraint)
{
    ...
    pe_rsc_trace(rsc_lh, "Processing colocation constraint between %s and %s", rsc_lh->id,
                 rsc_rh->id);
	/* with-rsc指定リソースのcolocation処理を実行する */
...
/* colocationのスコア情報をrsc指定側のリソース配置候補ノード(allowed_nodes)のスコアに反映する */
static void
colocation_match(resource_t * rsc_lh, resource_t * rsc_rh, rsc_colocation_t * constraint)
{
    ...
    while (g_hash_table_iter_next(&iter, NULL, (void **)&node)) {
        ...
        if (do_check && safe_str_eq(tmp, value)) {
			...
			/* with-rsc指定リソースの配置先が決定していないか、colocationのスコアがINFINITYの場合は */
			/* スコアのマイナス値をrsc指定の配置候補ノードのweightに加算する */
			/* --- これにより、with-rsc指定側のリソースの配置が決定していない間は、rsc指定側のリソースは ---*/
			/* --- 配置されない --- */
    ...
    if (can_run_any(work)
		...
		/* 処理後、rsc指定リソースの配置先が存在する場合 */
		/* 一旦、rsc指定リソースの配置候補ノードを解放する */
        g_hash_table_destroy(rsc_lh->allowed_nodes);
        /* 作成した配置候補ノードで、rsc指定リソースの配置候補ノードを更新する */
        rsc_lh->allowed_nodes = work;
        work = NULL;

    } else {
		/* 処理後、rsc指定リソースの配置先が存在しない場合 */
        char *score = score2char(constraint->score);
		/* Rolling back scoreのログを出力する(allowed_nodesは更新されない) */
