                 ...
                 const char *symmetrical, pe_working_set_t * data_set)
{
    ...
    /* resource_setに設定された"action","sequential","kind"を取得 */
    ...
    if (kind_s) {
		/* kindが指定されている場合はタイプを取得する */
        local_kind = get_ordering_type(set);
    }
    if (sequential_s == NULL) {
		/* sequentialが指定されていない場合は、"1"(true)をデフォルトにする */
        sequential_s = "1";
    }
	/* sequentialをbool化して、TRUE判定する */
	...
	/* resouce_set内のすべてのresource_refで参照されるリソースのリストを生成する */
    ...
    *rsc = NULL;
     ...
     *begin = get_pseudo_op(begin_id, data_set);
     ...
     free(end_id);
     */
	/* 生成したresouce_set内のリソースのリストをorderを処理する */
	/* 例
	 <rsc_order id="order-1" symmetrical="xxxxx" kind="xxxx">
 		<resource_set id="ordered-set-example" sequential="xxxx">
 		<resource_ref id="A"/>
 		<resource_ref id="B"/>
 		<resource_ref id="C"/>
 		</resource_set>
	...
	*/
    ...
    while (set_iter != NULL) {
        ...
        if (local_kind == pe_order_kind_serialize) {
            ...
            GListPtr gIter = NULL;
			/* serializeの場合には、resource_setの出現順に、N -> N+1,N+2,N+3...のすべてのorderを生成する */
			/* 例の場合、最終的に以下が生成される
				A->B
				A->C
				B->C
			*/
			...
			/* serialize以外の場合で、seqential値がtrueの場合は、resource_setの出現順に、N -> N+1のorderを生成する */
            if (last != NULL) {
			/* 例の場合、最終的に以下が生成される
				A->B
				B->C
    ...
    if (crm_is_true(symmetrical) == FALSE) {
		/* symmetrical値がfalseの場合は、逆順を生成しないので処理を終了する */
        goto done;

    } else if (symmetrical && local_kind == pe_order_kind_serialize) {
		/* symmetrical値がtrueでも、serializeの場合は処理を終了する */
        crm_config_warn("Cannot invert serialized constraint set %s", id);
        goto done;

    } else if (local_kind == pe_order_kind_serialize) {
		/* serializeの場合は処理を終了する */
        goto done;
    }
	/* 逆順のorder生成を開始 */
    last = NULL;
    action = invert_action(action);	/* 逆のactionを生成する */
	...
	/* 生成したresouce_set内のリソースのリストの逆順のorderを処理する */
    set_iter = resources;
    while (set_iter != NULL) {
        resource = (resource_t *) set_iter->data;
        set_iter = set_iter->next;
        ...
        if (sequential) {
			/* seqential値がtrueの場合は、resource_setの出現の逆順に、N+1->Nの逆順のorderを生成する */
			/* 例の場合、最終的に以下が逆順のorder生成される
				B->A
				C->B
			*/
            if (last != NULL) {
...
/* resource_set間のorder展開処理 */
/* 前に出現したresource_setがset1, 後ろがset2 */
static gboolean
               ...
               pe_working_set_t * data_set, gboolean invert, gboolean symmetrical)
{
	...
	/* 対象となる２つのresourece_setの"action", "serquential", 前に出現したresource_setの"require-all"値を取り出す */
	...
	/* invertパラメータがTRUEの場合は、逆順のアクションを生成 */
    if (invert) {
    ...
    if(safe_str_eq(RSC_STOP, action_1) || safe_str_eq(RSC_DEMOTE, action_1)) {
         ...
         */
        /* 前に出現したresouce_setのactionがSTOPか、DEMOTEの場合は、require_allをTRUEにする */
        require_all = TRUE;
    }
    ...
    if (!require_all) {
		...
		/* 前に出現したresource_setのresource_refをすべて処理する */
        for (xml_rsc = __xml_first_child(set1); xml_rsc != NULL; xml_rsc = __xml_next(xml_rsc)) {
			...
			/* 前に出現したresource_setのリソースのresource_refをリソース情報に変換 */
            ...
            /* 前に出現したresource_setのリソースとunordered_actionのorderを生成する */
			...
			/* 次に出現したresource_setのresource_refをすべて処理する */
            for (xml_rsc_2 = __xml_first_child(set2); xml_rsc_2 != NULL;
				...
				/* 次に出現したresource_setのリソースのresource_refをリソース情報に変換 */
                ...
                /* unordered_actionと次に出現したresource_setのリソースのorderを生成する */
    ...
    /* 前に出現したresource_setの"require-all"値がTRUEで指定されている場合 */
    if (crm_is_true(sequential_1)) {
		/* 前に出現したresource_setのsequantialがtrueの場合 */
        if (invert == FALSE) {
            /* get the last one */
            const char *rid = NULL;
            ...
            /* 前に出現したresouce_setの最後のリソースを取得する */
            EXPAND_CONSTRAINT_IDREF(id, rsc_1, rid);

        } else {
            /* get the first one */
            /* 前に出現したresouce_setの最初のリソースを取得する */
            for (xml_rsc = __xml_first_child(set1); xml_rsc != NULL; xml_rsc = __xml_next(xml_rsc)) {
                if (crm_str_eq((const char *)xml_rsc->name, XML_TAG_RESOURCE_REF, TRUE)) {
                    EXPAND_CONSTRAINT_IDREF(id, rsc_1, ID(xml_rsc));
                    break;
    ...
    if (crm_is_true(sequential_2)) {
        ...
        if (invert == FALSE) {
            ...
            /* get the last one */
            /* 後に出現したresouce_setの最後のリソースを取得する */
            const char *rid = NULL;
	 ...
	 * 正準(呼び出し時はそのまま）
	 ...
	 <rsc_order id="order-2" symmetrical="xxxxx" kind="xxxx">
 		<resource_set id="ordered-set-example" sequential="xxxx">
 		<resource_ref id="X"/>
 		<resource_ref id="Y"/>
 		</resource_set>
	...
	*/
    if (rsc_1 != NULL && rsc_2 != NULL) {
		/* 前・後のresourece_setのsequentialがTRUEの場合 */
		/* 例の場合 :
		   	正準の場合
		   		B->X
		   	逆順の場合
		...
		*/
		...
		/* 前のresourece_setのみがsequentialがTRUEの場合 */
        for (xml_rsc = __xml_first_child(set2); xml_rsc != NULL; xml_rsc = __xml_next(xml_rsc)) {
            if (crm_str_eq((const char *)xml_rsc->name, XML_TAG_RESOURCE_REF, TRUE)) {
                EXPAND_CONSTRAINT_IDREF(id, rsc_2, ID(xml_rsc));
                new_rsc_order(rsc_1, action_1, rsc_2, action_2, flags, data_set);
		...
		*/
		...
		/* 後ろのresourece_setのみがsequentialがTRUEの場合 */
        xmlNode *xml_rsc = NULL;
		...
		*/
		...
		/* 前・後のresourece_setのsequentialがFALSEの場合 */
        for (xml_rsc = __xml_first_child(set1); xml_rsc != NULL; xml_rsc = __xml_next(xml_rsc)) {
            if (crm_str_eq((const char *)xml_rsc->name, XML_TAG_RESOURCE_REF, TRUE)) {
                xmlNode *xml_rsc_2 = NULL;
        ...
        if (crm_str_eq((const char *)set->name, XML_CONS_TAG_RSC_SET, TRUE)) {
            ...
            if (unpack_order_set(set, kind, &rsc, &set_begin, &set_end,
                 ...
                 */
                          ...
                          last) {
				/* 前後のresource_set間のorderを展開 */
				/* ただし、invert_boolは最初に出現したresouce_setを含んだrsc_orderのsymmetricalのみが渡される */
                if (order_rsc_sets(id, last, set, kind, data_set, FALSE, invert_bool) == FALSE) {
                    return FALSE;
                }
				/* 最初に出現したresouce_setを含んだrsc_orderのsymmetricalがtrueの場合は、反転のresource_set間のorderを展開する */
 				/* ただし、invert_boolは最初に出現したresouce_setを含んだrsc_orderのsymmetricalのみが渡される */
                if (invert_bool
...
unpack_colocation_set(xmlNode * set, int score, pe_working_set_t * data_set)
{
    ...
    if (sequential != NULL && crm_is_true(sequential) == FALSE) {
		/* sequentialがFALSEの場合は、colocationは生成しない */
		/* 未設定の場合は、TRUEで処理 */
        return TRUE;
...
/* 例
      <rsc_colocation id="rsc_colocation-grpPg-clnPing" score="INFINITY">
        <resource_set id="rsc_colocation-grpPg-clnPing-0" sequential="xxxx">
          <resource_ref id="A"/>
          <resource_ref id="B"/>
        </resource_set>
        <resource_set id="rsc_colocation-grpPg-clnPing-1" sequential="xxxx">
          <resource_ref id="X"/>
          <resource_ref id="Y"/>
        </resource_set>
      </rsc_colocation>
...
*/
    if (crm_is_true(sequential_1)) {
		/* 先に処理をしたresource_setのsequentialがtrueの場合は、*/
		/* 先に処理したresource_setの先頭リソースの情報を取得する */
        /* get the first one */
        for (xml_rsc = __xml_first_child(set1); xml_rsc != NULL; xml_rsc = __xml_next(xml_rsc)) {
            if (crm_str_eq((const char *)xml_rsc->name, XML_TAG_RESOURCE_REF, TRUE)) {
                EXPAND_CONSTRAINT_IDREF(id, rsc_1, ID(xml_rsc));
    ...
    if (crm_is_true(sequential_2)) {
		/* 次に処理をしたresource_setのsequentialがtrueの場合は、*/
		/* 次に処理したresource_setの最後リソースの情報を取得する */
        /* get the last one */
        const char *rid = NULL;
    ...
    if (rsc_1 != NULL && rsc_2 != NULL) {
		/* 先に処理をしたresource_setも、次に処理をしたresource_setもsequentialがtrueの場合は */
		/* colocation 先に処理したresource_setの先頭リソース  with 次に処理をしたresource_setの最後のリソース */
		/* の制約を生成する */
		/* 例の場合で、両方ともsequential=trueだったとすると */
		/* colocation A with Y */
		
        rsc_colocation_new(id, NULL, score, rsc_1, rsc_2, role_1, role_2, data_set);

    } else if (rsc_1 != NULL) {
		/* 先に処理をしたresource_setのsequentialがtrueで、次に処理をしたresource_setもsequentialがfalseの場合は */
		/* 先に処理したresource_setの先頭リソースと次に処理をしたresource_setのすべてのリソースとのcolocationの制約を生成する */
		/* 例の場合で、両方ともsequential=true/falseだったとすると */
		/* colocation A with X */
		/* colocation A with Y */		
        for (xml_rsc = __xml_first_child(set2); xml_rsc != NULL; xml_rsc = __xml_next(xml_rsc)) {
            if (crm_str_eq((const char *)xml_rsc->name, XML_TAG_RESOURCE_REF, TRUE)) {
		...
		/* 先に処理をしたresource_setのsequentialがfalseで、次に処理をしたresource_setもsequentialがtrueの場合は */
		/* 次に処理したresource_setの最後リソースと先に処理をしたresource_setのすべてのリソースとのcolocationの制約を生成する */
		/* 例の場合で、両方ともsequential=false/trueだったとすると */
		/* colocation A with Y */
		/* colocation B with Y */
        for (xml_rsc = __xml_first_child(set1); xml_rsc != NULL; xml_rsc = __xml_next(xml_rsc)) {
            if (crm_str_eq((const char *)xml_rsc->name, XML_TAG_RESOURCE_REF, TRUE)) {
		...
		/* どちらのresource_setのsequenatialもfalseだった場合 */
		/* 先に処理をしたresource_setのすべてのリソースと次に処理をしたresource_setのすべてのリソースとのcolocationの制約を生成する */
		/* 例の場合で、両方ともsequential=false/falseだったとすると */
		/* colocation A with X */
		/* colocation A with Y */
		/* colocation B with X */
		/* colocation B with Y */
